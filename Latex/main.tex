...
\begin{figure}[h]
\centering
\includegraphics[scale=0.8]{gradnetz.jpg}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.3]{reachdiag.png}
...
$$Ird_{minPts}(p) = (\frac{\sum\limits_{o \in N_{minPts}(p)} rd_{minPts}(p,o)}{|N_{minPts}(p)|})^{-1}$$
...
\begin{equation}
y_i \cdot (\omega \cdot x_i + b) \geq 1
\end{equation}

Unter dieser Nebenbedingung lautet die Lagrange-Funktion:
\begin{equation} \label{lagrange}
\mathcal{L}(\vec{\omega},b,\vec{\alpha}) = \frac{1}{2}||\vec{\omega}||^2 - \sum_{i=1}^n \alpha_i \cdot [y_i \cdot (\vec{\omega} \cdot \vec{x_i} + b) - 1]
\end{equation}
...
\mathcal{L}(\vec{\omega},b,\vec{\alpha}) = \frac{1}{2}(\sum_{i=1}^n \alpha_i \cdot y_i \cdot \vec{x_i})(\sum_{j=1}^n \alpha_j \cdot y_j \vec{x_j}) - (\sum_{i=1}^n \alpha_i \cdot y_i \cdot \vec{x_i}) \cdot (\sum_{j=1}^n \alpha_j \cdot y_j \cdot \vec{x_j}) - \sum_{i=1}^n \alpha_i \cdot y_i \cdot b + \sum_{i=1}^n \alpha_i
\end{equation}
...
Diese lässt sich unter Nebenbedingung \eqref{Lpb} und $\alpha_i \geq 0$ maximieren.
...
\includegraphics[scale=0.43]{svm2.png}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{svm3.png}
\end{figure}
...
%Outlier Detection z.B. zum Finden von Erdbeeben
%Siehe Tab n Browser zu arcgis Werkzeugen Antwendungsgebiete 
